...
import { RateLimiter, globalRateLimiter } from './utils/rate-limiter.js';
...
export class CodeReviewOrchestrator {
    ...
    async reviewPullRequest(owner, repo, prNumber) {
        ...
        if (!model) {
            throw new ReviewError('ANTHROPIC_MODEL environment variable is required', ErrorCodes.INVALID_CONFIG);
        }
        const startedAt = Date.now();
        logger.info('Starting code review', { owner, repo, prNumber });
        await this.rateLimiter.acquire(ESTIMATED_TOKENS_PER_REVIEW);
        try {
            const structuredOutput = await withRetry(() => withTimeout(() => this.runQuery(owner, repo, prNumber, model), ORCHESTRATOR_TIMEOUT_MS, `Review of ${owner}/${repo}#${prNumber} timed out`), 3, 1000);
            const parsed = ReviewReportSchema.safeParse(structuredOutput);
            if (!parsed.success) {
                throw new ReviewError(`Orchestrator output failed schema validation: ${parsed.error.message}`, ErrorCodes.STRUCTURED_OUTPUT_FAILED, { issues: parsed.error.issues });
            }
    ...
    async runQuery(owner, repo, prNumber, model) {
        ...
        if (!structuredOutput) {
            throw new ReviewError(failureSubtype
                ? `Orchestrator run ended with SDK failure subtype "${failureSubtype}"`
                : 'Orchestrator did not produce a structured_output payload', ErrorCodes.AGENT_FAILED, { owner, repo, prNumber, failureSubtype });
        }
